...
### Example: Run as a Kubernetes CronJob
      ...
      template:
        spec:
          ...
          volumes:
            - name: csv-output
              hostPath:
                path: /var/lib/container-security-csv
...
#### CSV Log Rotation Example

Add a **second CronJob** to rotate the CSV file once per day:

```yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: container-security-csv-rotate
  namespace: monitoring
spec:
  schedule: "59 23 * * *"  # Every day at 23:59
  jobTemplate:
    spec:
      template:
        spec:
          ...
          containers:
            - name: rotate
              ...
              command:
                ...
                - -c
                - |
                  mv /var/lib/container-security-csv/adoption.csv /var/lib/container-security-csv/adoption-$(date +\%Y-\%m-\%d).csv || true
          volumes:
            - name: csv-output
              hostPath:
                path: /var/lib/container-security-csv
